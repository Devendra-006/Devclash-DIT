...
- **openrouter** (default): cloud models via OpenRouter.
- **ollama**: local models via Ollama (`/v1` compatibility).

Set ``LLM_PROVIDER=ollama`` and ``OLLAMA_MODEL`` to use a local model.
All prompts are copied verbatim from PRD Section 5.1, 5.2, 5.3.
"""
...
def get_llm_provider() -> str:
    """``openrouter`` (default) or ``ollama``."""
    p = os.getenv("LLM_PROVIDER", "openrouter").strip().lower()
...
def _call_claude(system_prompt: str, user_prompt: str, max_retries: int = 3) -> Dict[str, Any]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            raise RuntimeError(f"{backend} request timed out: {e}") from e
        except RateLimitError as e:
            last_error = e
            wait = 5 * (2 ** attempt)  # 5s, 10s, 20s
            print(f"[claude_engine] Rate limited (attempt {attempt+1}/{max_retries}), retrying in {wait}s…")
            time.sleep(wait)
...
def code_review(
    ...
    # Select top files by size (most impactful)
    top_files = sorted(files, key=lambda f: f.get("line_count", 0), reverse=True)[:8]
...
Files to review (top {len(top_files)} by impact score):
...
def architecture_summarizer(
    ...
    # Serialize graph for the prompt
    graph_json = json.dumps({"nodes": graph_data["nodes"], "edges": graph_data["edges"]}, indent=2)
...
def career_verdict(
    ...
    findings_json = json.dumps(aggregated_findings[:50], indent=2)  # Cap findings
