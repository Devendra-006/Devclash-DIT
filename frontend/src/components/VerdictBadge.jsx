      ...
      {topEvidence && topEvidence.length > 0 && (
        <div className="border-t border-gray-200 pt-4">
          ...
          >
            ...
            <span className={`transform transition-transform ${showEvidence ? 'rotate-180' : ''}`}>
              ▾
            </span>
          </button>
          ...
          {showEvidence && (
            <div className="mt-3 space-y-2">
              {topEvidence.slice(0, 5).map((evidence, idx) => (
                <div 
                  key={idx} 
                  className="flex items-start gap-2 p-2 bg-white rounded-lg cursor-pointer hover:bg-gray-50"
